...
const seedCurriculumLabs = async () => {
  try {
    console.log("🎓 Creating curriculum-based AIML labs...\n");

    // Clear existing labs
    await Lab.deleteMany({});
    console.log("🗑️ Cleared existing labs");

    // Get modules and skills
    const modules = await Module.find();
    const skills = await Skill.find();
    
    console.log(`✅ Found ${modules.length} modules and ${skills.length} skills`);

    const labsData = [];

    // Lab 1: Forward Feature Selection (Unit I)
    labsData.push({
      name: "Forward Feature Selection with KDD Cup99",
      description: "Apply forward feature selection technique to reduce dimensionality of KDD Cup99 dataset by incrementally selecting features that improve model performance.",
      objectiveText: "Implement forward feature selection to reduce irrelevant or redundant features while maintaining predictive power.",
      difficulty: 2,
      estimatedTime: 90,
      tags: ["feature-selection", "dimensionality-reduction", "kdd-cup99", "sklearn"],
      scenario: "both",
      environment: "simulated",
      skillId: skills.find(s => s.name === "Feature Engineering")?._id,
      moduleId: modules.find(m => m.title.includes("Feature Selection"))?._id,
      content: {
...
if __name__ == "__main__":
    ...
    // Lab 2: Backward Feature Elimination (Unit I)
    labsData.push({
      name: "Backward Feature Elimination with KDD Cup99",
      ...
      objectiveText: "Use backward elimination to remove irrelevant features while maintaining dataset effectiveness.",
      difficulty: 2,
      estimatedTime: 90,
...
if __name__ == "__main__":
    ...
    // Lab 3: Linear Regression (Unit II)
    labsData.push({
      name: "Linear Regression for House Price Prediction",
      ...
      objectiveText: "Implement linear regression for regression tasks with preprocessing and evaluation.",
      difficulty: 1,
      estimatedTime: 60,
      tags: ["linear-regression", "house-prediction", "regression", "scikit-learn"],
      scenario: "both",
      environment: "simulated",
      skillId: skills.find(s => s.name === "Regression Analysis")?._id,
      moduleId: modules.find(m => m.title.includes("Supervised"))?._id,
      content: {
...
if __name__ == "__main__":
    ...
    // Lab 4: Logistic Regression (Unit II)
    labsData.push({
      ...
      objectiveText: "Master logistic regression for binary and multi-class classification with evaluation metrics.",
      difficulty: 2,
      estimatedTime: 75,
      tags: ["logistic-regression", "classification", "iris", "startups"],
      scenario: "both",
      environment: "simulated",
      skillId: skills.find(s => s.name === "Classification Algorithms")?._id,
      moduleId: modules.find(m => m.title.includes("Supervised"))?._id,
      content: {
...
def classify_startups():
    ...
    data = {
        'R&D_Spend': [165349.20, 162597.70, 153441.36, 144372.41, 142107.34],
        'Administration': [136897.80, 151377.59, 101145.55, 118671.85, 91391.77],
        'Marketing_Spend': [471784.10, 443898.53, 407934.54, 383199.62, 366168.42],
        'State': ['New York', 'California', 'Florida', 'New York', 'California'],
        'Profit': [192261.83, 191792.06, 191050.39, 182901.91, 166187.94]
    }
...
if __name__ == "__main__":
    ...
    // Lab 5: Decision Tree ID3 (Unit II)
    labsData.push({
      ...
      objectiveText: "Build ID3 algorithm from scratch and understand decision tree construction.",
      difficulty: 3,
      estimatedTime: 90,
...
if __name__ == "__main__":
    ...
    // Lab 6: K-Nearest Neighbors (Unit II)
    labsData.push({
      name: "K-Nearest Neighbors (KNN) Algorithm",
      description: "Implement KNN algorithm for IRIS and Car Evaluation datasets with k-optimization.",
      objectiveText: "Master KNN classification and optimize hyperparameters for better accuracy.",
      difficulty: 2,
      estimatedTime: 80,
...
if __name__ == "__main__":
    ...
    // Lab 7: Hierarchical Clustering (Unit III)
    labsData.push({
      name: "Hierarchical Clustering with Wholesale Customers",
      description: "Implement agglomerative hierarchical clustering on Wholesale Customers dataset with dendrogram visualization.",
      objectiveText: "Master hierarchical clustering and evaluate cluster quality using silhouette scores.",
      difficulty: 3,
      estimatedTime: 85,
      tags: ["hierarchical-clustering", "wholesale-customers", "dendrogram", "silhouette"],
      scenario: "both",
      environment: "simulated",
      skillId: skills.find(s => s.name === "Clustering Algorithms")?._id,
      moduleId: modules.find(m => m.title.includes("Unsupervised"))?._id,
      content: {
...
if __name__ == "__main__":
    ...
    // Lab 8: K-Means Clustering (Unit III)
    labsData.push({
      name: "K-Means Clustering with Wholesale Customers",
      description: "Implement K-Means clustering on Wholesale Customers dataset with elbow method optimization.",
      objectiveText: "Master K-Means algorithm and determine optimal clusters using elbow method.",
      difficulty: 2,
      estimatedTime: 70,
...
if __name__ == "__main__":
    ...
    // Lab 9: Gradient Boosting (Unit IV)
    labsData.push({
      name: "Gradient Boosting Implementation",
      description: "Implement Gradient Boosting for classification on IRIS and 50_Startups datasets with hyperparameter tuning.",
      objectiveText: "Master Gradient Boosting ensemble method and optimize hyperparameters for better performance.",
      difficulty: 4,
      estimatedTime: 95,
      tags: ["gradient-boosting", "ensemble", "classification", "hyperparameter-tuning"],
      scenario: "both",
      environment: "simulated",
      skillId: skills.find(s => s.name === "Ensemble Methods")?._id,
      moduleId: modules.find(m => m.title.includes("Ensemble"))?._id,
      content: {
...
if __name__ == "__main__":
    ...
    // Lab 10: XGBoost Implementation (Unit IV)
    labsData.push({
      name: "XGBoost Advanced Implementation",
      description: "Implement XGBoost for classification on IRIS and 50_Startups with advanced hyperparameter optimization.",
      objectiveText: "Master XGBoost ensemble method and optimize hyperparameters for maximum accuracy.",
      difficulty: 4,
      estimatedTime: 100,
...
if __name__ == "__main__":
    ...
    console.log(`✅ Created ${createdLabs.length} curriculum-based AIML labs`);

    console.log("\n🔬 Lab Summary:");
    createdLabs.forEach((lab, index) => {
      console.log(`${index + 1}. ${lab.name} (Difficulty: ${lab.difficulty}/4, ${lab.estimatedTime}min)`);
    });

    console.log("\n🎉 Curriculum labs creation completed!");
