...
export const seedSimpleCareerRoadmaps = async () => {
  try {
    console.log("🛤️ Starting simple career roadmap seeding...");
    ...
    for (const roadmapData of simpleCareerRoadmaps) {
      const roadmap = new CareerRoadmap(roadmapData);
      await roadmap.save();
      createdCount++;
      console.log(`✅ Created career roadmap: ${roadmapData.roleName}`);
    }

    console.log(`🎉 Simple career roadmap seeding completed! Created ${createdCount} roadmaps.`);
    console.log("\n🛤️ Available Career Paths:");
    simpleCareerRoadmaps.forEach((roadmap, index) => {
    ...
    console.error("❌ Error seeding simple career roadmaps:", error);
    return false;
  }
};
